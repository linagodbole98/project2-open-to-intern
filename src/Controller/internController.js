...
const CreateIntern = async function (req, res) {
    try {
        ...
        if (!validator.isValidEmail(email)) return res.status(400).send({status:false, msg: `this Email is not valid ::${email}` })
        ...
        if (!validator.isValidNumber(mobile)) return res.status(400).send({ status:false,msg: `this mobile is not valid ::${mobile}` })
